...
def start():
    ...
    if not collecting_data:
        ...
        csv_file = f"mpu6050_data_{timestamp}.csv"
    return redirect(url_for("index"))
...
def stop():
    ...
    if collecting_data:
        collecting_data = False
        # Save data to CSV (adding the label column)
        df = pd.DataFrame(
            data, 
...
def receive_data():
    ...
    if collecting_data:
        ...
        entry = [
            timestamp,
            content.get("accel_x", 0),
            content.get("accel_y", 0),
            content.get("accel_z", 0),
            content.get("gyro_x", 0),
            content.get("gyro_y", 0),
            content.get("gyro_z", 0),
