...
void loop() {
    ...
    // Normalize IR value to range 0.0–1.0
    float irNormalized = constrain((ir - 500.0) / (150000.0 - 500.0), 0.0, 1.0);

    // Map to RGB: Blue → Green → Red
    uint8_t red = (uint8_t)(irNormalized * 255);
    uint8_t green = (uint8_t)((1.0 - abs(irNormalized - 0.5) * 2) * 255);
    uint8_t blue = (uint8_t)((1.0 - irNormalized) * 255);

    // Set color with brightness (optional tweak)
    uint8_t brightness = 20; // max is 255, adjust as needed
